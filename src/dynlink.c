 ...
 *  USA
 *
 * $Id: dynlink.c 606 2006-06-08 22:35:55Z stu $
 *
 */
#include "stdinc.h"
...
{
  printf( "Undefined symbol: %s", symbolName);
  return;
}
...
multipleErrorHandler(NSSymbol s, NSModule old, NSModule new)
{
  ...
  ** module, maybe?
  */
  printf("Symbol `%s' found in `%s' and `%s'",
...
load_a_module(char *path, int warn, int core)
{
  ...
  if (shl_findsym(&tmpptr, "_moddeinit", TYPE_UNDEFINED, (void *)&mod_deinit) == -1)
  {
    ...
    {
      printf( "Module %s has no _moddeinit() function",
                           mod_basename);
  ...
  if (warn == 1)
  {
    printf( "Module %s [version: %s] loaded at %p", mod_basename, 
        ver, addr);
  }
